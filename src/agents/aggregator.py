...
class Aggregator(BaseAgent):
    def __init__(self):
        super().__init__(model=ModelConfig.MODEL_AGGREGATOR)

    @time_it
    async def aggregate(self, session_data: Dict[str, Any], last_intent: str, pending_questions: Any = None, session_id: str = None) -> str:
        """
        汇总所有信息，生成给用户的回复。
