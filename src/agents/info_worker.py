...
class InfoWorker(BaseAgent):
    def __init__(self):
        ...
        super().__init__(model=ModelConfig.MODEL_INFO_WORKER, tools=[])
    ...
    async def process(self, user_input: str, current_data: Dict[str, Any], on_tool_call=None, session_id: str = None) -> Dict[str, Any]:
        ...
        result = await self.run_chain(system_prompt, user_prompt, on_tool_call=on_tool_call, session_id=session_id)
